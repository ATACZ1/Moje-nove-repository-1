        ...
        <div className="grid md:grid-cols-2 gap-16 md:gap-24 items-center">
          ...
          <div className="relative">
            ...
            <div className="absolute -bottom-6 -right-6 bg-sand-700 text-white p-6 hidden md:block">
              <div className="text-3xl font-serif font-light">65</div>
              <div className="text-xs tracking-widest uppercase font-sans font-light opacity-80 mt-1">
                míst v restauraci
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-sand-50 border border-sand-200 p-6 mb-8">
              <div className="text-xs tracking-widest uppercase font-sans font-light text-sand-500 mb-3">
                Polední menu (Po – Pá)
              </div>
              <div className="flex items-baseline gap-2 mb-1">
                <span className="font-serif font-light text-2xl text-sand-700">145 Kč</span>
                <span className="text-xs text-sand-400">polévka + hlavní jídlo</span>
              </div>
            ...
            {/* Beers on tap */}
            <div className="mb-8">
              <div className="text-xs tracking-widest uppercase font-sans font-light text-sand-400 mb-3">
                Na čepu
              </div>
              <div className="flex flex-wrap gap-2">
                {beers.map((b) => (
                  <span
                    key={b}
                    className="text-xs text-sand-600 border border-sand-200 px-3 py-1 font-light"
                  >
                    {b}
                  </span>
                ))}
            ...
            {/* Opening hours */}
            <div>
              <div className="text-xs tracking-widest uppercase font-sans font-light text-sand-400 mb-4">
                Otevírací doba
              </div>
              <div className="space-y-3">
                {hours.map((h) => (
                  <div key={h.day} className="flex flex-col sm:flex-row sm:justify-between text-sm">
                    <span className="font-sans font-light text-sand-700 min-w-[180px]">{h.day}</span>
                    <span className="font-sans font-light text-sand-500">{h.open}</span>
                    <span className="font-sans font-light text-sand-400 text-xs mt-1 sm:mt-0 sm:ml-4">
                      kuchyně {h.kitchen}
                    </span>
                  </div>
                ))}
